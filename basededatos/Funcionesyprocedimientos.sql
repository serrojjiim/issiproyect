...
W_DIASVACACIONES IN EMPLEADO.DIASVACACIONES%TYPE, W_OCULTO IN EMPLEADO.OCULTO%TYPE )



-- LE HE PUESTO LO DE OCULTO CUIDAO SOCIO!!!!!!!!
...
BEGIN
    ...
    FECHACONTRATACION = W_FECHACONTRATACION, DIASVACACIONES = W_DIASVACACIONES, OCULTO=W_OCULTO WHERE W_OID_EMP = OID_EMP;
    COMMIT WORK;
EXCEPTION
    WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('Se ha producido un error al actualizar la máquina asignada al empleado '||TIPO_EMPLEADO.NOMBRE||' '||TIPO_EMPLEADO.APELLIDOS||' ');
...
, W_NOMBRE IN CLIENTE.NOMBRE%TYPE, W_DIRECCION IN CLIENTE.DIRECCION%TYPE, W_TELEFONO IN CLIENTE.TELEFONO%TYPE, W_EMAIL IN CLIENTE.EMAIL%TYPE,W_OCULTO IN CLIENTE.OCULTO%TYPE)
...
BEGIN
    ...
    UPDATE CLIENTE
    SET CIF = W_CIF , NOMBRE = W_NOMBRE, DIRECCION = W_DIRECCION, TELEFONO = W_TELEFONO , EMAIL=W_EMAIL, OCULTO=W_OCULTO WHERE  W_OID_CLI=OID_CLI;
    COMMIT WORK;
EXCEPTION
    WHEN OTHERS THEN
...
CREATE OR REPLACE PROCEDURE ELIMINAR_CLIENTE(W_CIF IN CLIENTE.CIF%TYPE)
AS
BEGIN
DELETE FROM CLIENTE WHERE CIF=W_CIF;
EXCEPTION
    WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('Se ha producido un error al eliminar el cliente con CIF '||w_CIF||' ');
