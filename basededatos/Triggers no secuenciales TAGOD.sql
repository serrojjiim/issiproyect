...
BEGIN
    IF :NEW.stock < 1500
    THEN DBMS_OUTPUT.PUT_LINE('�ATENCI�N! El stock actual del material "'||:NEW.nombre||'" es de '||:NEW.stock||' unidades. Se debe realizar un pedido de material.');
    END IF;
END;
/
--RN15
...
EXCEPTION
    ...
    RAISE_APPLICATION_ERROR(-20100, 'No puede haber 2 jefes en una sola m�quina. Error provocado por el empleado: '||:NEW.nombre||' '||:NEW.apellidos);
...
BEGIN
    ...
    IF (:NEW.cargo between 1 and 8 OR :NEW.cargo = 11) AND (:NEW.oid_maq is not null) THEN
        RAISE_APPLICATION_ERROR(-20102, 'S�lo los peones y jefes de m�quina pueden trabajar en una m�quina. Error provocado por el empleado: '||:NEW.nombre||' '||:NEW.apellidos||' con cargo: '||w_cargo);
    END IF;
END;
/
--RN09A
...
CREATE OR REPLACE TRIGGER ckFechasPC_BIU
    ...
    IF :NEW.fechafinfabricacion <= :NEW.fechapedido THEN
        RAISE_APPLICATION_ERROR(-20104,'La fecha de fin de fabricaci�n no puede ser menor que la fecha de pedido');
    END IF;
    IF :NEW.fechaenvio <= :NEW.fechafinfabricacion THEN
        RAISE_APPLICATION_ERROR(-20104,'La fecha de env�o no puede ser menor que la fecha de fabricaci�n');
        END IF;
    IF :NEW.fechallegada <= :NEW.fechaenvio THEN
        RAISE_APPLICATION_ERROR(-20104,'La fecha de llegada no puede ser menor que la fecha de env�o');
    END IF;
...
CREATE OR REPLACE TRIGGER distanciaFechasPedCli_BIU
    ...
    IF :NEW.fechafinfabricacion IS NULL AND SYSTIMESTAMP - :NEW.fechapedido > 1 THEN
        DBMS_OUTPUT.PUT_LINE('�ATENCI�N! El pedido con fecha "'||:NEW.fechapedido||'" y coste total "'||:NEW.costetotal||'" debe ser enviado inmediatamente.');
    END IF;
END;
/
--RN07
CREATE OR REPLACE TRIGGER distanciaFechasPedCliPago_BIU
    ...
    IF :NEW.fechapago IS NULL AND SYSTIMESTAMP - :NEW.fechallegada > 7 THEN
        DBMS_OUTPUT.PUT_LINE('�ATENCI�N! El pedido del cliente "'||:NEW.oid_cli||'" y coste total "'||:NEW.costetotal||'" debe ser pagado inmediatamente.');
    END IF;
END;
/
...
BEGIN
    ...
    IF w_cargo != 11 THEN
        RAISE_APPLICATION_ERROR(-20105,'S�lo los camioneros pueden conducir un cami�n. Error provocado por el empleado: '||w_nombre||' '||w_apellidos||', con cargo: '||w_rol||'.');     
    END IF;
END;
/
--RN13
...
BEGIN
    ...
    SELECT rol INTO w_rol FROM cargo WHERE cargo.oid_cargo = w_cargo;
    IF w_cargo != 6 THEN
        RAISE_APPLICATION_ERROR(-20105,'S�lo los gerentes de compras pueden realizar un pedido a proveedor. Error provocado por el empleado: '||w_nombre||' '||w_apellidos||', con cargo: '||w_rol||'.');     
    END IF;
END;
/
--RN12
